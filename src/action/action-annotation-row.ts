...
export function init() {
    ...
    async function getHtml() {
        ...
        const attachments = Zotero.Items.get(item.getAttachments()).filter((i) => i.isPDFAttachment() || i.isSnapshotAttachment() || i.isEPUBAttachment() || i.isWebAttachment());
        let res = `<h1>${item.getField("title")}</h1>`;
        const allAnnotations = []; // 在函数作用域内声明数组来收集所有注释
        for (const attachment of attachments) {
    ...
    async function parseAnnotationHTML(annotations: Zotero.Item[], options = { noteItem: Zotero.Item }) {
        ...
        for (const ann of annotations) {
            ...
            if (annJson)
                annotationJSONList.push(annJson);
        }
