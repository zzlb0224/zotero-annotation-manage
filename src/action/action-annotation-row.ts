...
export function init() {
  ...
  function unregisteredID_showAnnotations() {
    return Zotero.ItemPaneManager.unregisterSection(
      "zoterotag-euclpts-com-custom-section-example",
    );
  }
  ...
  function appendAnnotation(root: HTMLDivElement, item: Zotero.Item) {
    ...
    icon.setAttribute(
      "src",
      `chrome://zotero/skin/16/universal/annotate-${item.annotationType}.svg`,
    );
  ...
  async function importAnnotationImagesToNote(
    note: Zotero.Item,
  ...
  async function parseAnnotationHTML(
    annotations: Zotero.Item[],
    options = { noteItem: Zotero.Item },
  ) {
    ...
    return Zotero.EditorInstanceUtilities.serializeAnnotations(
      annotationJSONList,
      true,
    );
  }
