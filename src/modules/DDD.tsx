...
import { getPopupWin, getSelectedItems } from './menu';

export async function DDDTagClear() {
    const ProgressWindow = ztoolkit.ProgressWindow, d1p = getPref("date-1-pre"), d2p = getPref("date-2-pre"), d121 = getPref("date-1-2-1-pre"), d1210 = getPref("date-1-2-10-pre"), d1230 = getPref("date-1-2-30-pre");
...
export async function DDDTagSet(collectionOrItem: "collection" | "item") {
    ...
    const ProgressWindow = ztoolkit.ProgressWindow, d1s = getPref("date-1") as string, d2s = getPref("date-2") as string, d1p = getPref("date-1-pre"), d2p = getPref("date-2-pre"), d121 = getPref("date-1-2-1-pre"), d1210 = getPref("date-1-2-10-pre"), d1230 = getPref("date-1-2-30-pre");
    ...
    const regExpDate = /\d{1,2}[\s-]+(?:Jan|January|Feb|February|Mar|March|Apr|April|May|Jul|July|Jun|June|Aug|August|Sep|September|Oct|October|Nov|November|Dec|December)[\s-]+\d{2,4}/;
    const ids = items
    ...
    for (let index = 0; index < pdfs.length; index++) {
        ...
        let text = "", extractedPages = 0, totalPages = 0;
        try {
